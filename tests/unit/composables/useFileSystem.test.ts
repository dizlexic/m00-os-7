...
describe('useFileSystem', () => {
  ...
  it('should get node by path', () => {
    ...
    expect(fs.getNodeByPath('Macintosh HD/System Folder/Finder')?.id).toBe(file.id)
    expect(fs.getNodeByPath('Macintosh HD/System Folder')?.id).toBe(folder.id)
    expect(fs.getNodeByPath('Macintosh HD')?.id).toBe(root.id)
    expect(fs.getNodeByPath('')).toBeUndefined()
    expect(fs.getNodeByPath('Wrong HD/System Folder')).toBeUndefined()
    expect(fs.getNodeByPath('Macintosh HD/Nonexistent')).toBeUndefined()
  })

  it('should export getTrash and moveToTrash', () => {
    expect((fs as any).getTrash).toBeDefined()
    expect(typeof (fs as any).getTrash).toBe('function')
    expect((fs as any).moveToTrash).toBeDefined()
